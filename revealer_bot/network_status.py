...
async def get_lynx_network_status(interaction):
    ...
    status_response = requests.get("https://lynx-1.nucypher.network:9151/status/?json=true", verify=False)
    ...
    reply_awaitable = thread.send(f"Fleet state for {network_name} is:\n **{fleet_state_name}** ({fleet_state_checksum})")
    ...
    if down_nodes:
        summary += "## Down Nodes:\n"
        summary += "".join(f"* {node['nickname']['text']} ({node['rest_url']})\n" for node in up_nodes)
    if up_nodes:
        summary += "----\n"
        summary += "**Up Nodes**:\n"
        summary += "".join(f"* {node['nickname']['text']} ({node['rest_url']})\n" for node in up_nodes)
